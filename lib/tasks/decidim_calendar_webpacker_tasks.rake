# frozen_string_literal: true

require "decidim/gem_manager"

namespace :decidim_calendar do
  namespace :webpacker do
    desc "Installs Decidim Calendar webpacker files in Rails instance application"
    task install: :environment do
      raise "Decidim gem is not installed" if decidim_path.nil?

      install_calendar_npm
    end

    desc "Adds Decidim Calendar dependencies in package.json"
    task upgrade: :environment do
      raise "Decidim gem is not installed" if decidim_path.nil?

      install_calendar_npm
    end

    def install_calendar_npm
      calendar_npm_dependencies.each do |type, packages|
        puts "install NPM packages. You can also do this manually with this command:"
        puts "npm i --save-#{type} #{packages.join(" ")}"
        system! "npm i --save-#{type} #{packages.join(" ")}"
      end
    end

    def calendar_npm_dependencies
      @calendar_npm_dependencies ||= begin
        package_json = JSON.parse(File.read(calendar_path.join("package.json")))

        {
          prod: package_json["dependencies"].map { |package, version| "#{package}@#{version}" },
          dev: package_json["devDependencies"].map { |package, version| "#{package}@#{version}" }
        }.freeze
      end
    end

    def calendar_path
      @calendar_path ||= Pathname.new(calendar_gemspec.full_gem_path) if Gem.loaded_specs.has_key?("decidim-calendar")
    end

    def calendar_gemspec
      @calendar_gemspec ||= Gem.loaded_specs["decidim-calendar"]
    end

    def rails_app_path
      @rails_app_path ||= Rails.root
    end

    def system!(command)
      system("cd #{rails_app_path} && #{command}") || abort("\n== Command #{command} failed ==")
    end
  end
end
